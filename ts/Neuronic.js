...
var Point = /** @class */ (function () {
    ...
    Point.RandomBetween = function (point1, point2) {
    ...
    Point.scaled = function (thePoint, scale) {
        var ret = new Point();
...
var Vector2D = /** @class */ (function () {
    ...
    Object.defineProperty(Vector2D.prototype, "normalised", {
        get: function () {
            var len = this.length;
    ...
    Vector2D.prototype.scaled = function (scale) {
        var ret = new Vector2D();
        ret.x = this.x * scale;
        ret.y = this.y * scale;
        return ret;
    ...
    Vector2D.AngleBetween2Vectors = function (v1, v2) {
        var dp = v1.x * v2.x + v1.y * v2.y;
        return Math.acos(dp / Math.sqrt(v1.lengthSquared * v2.lengthSquared));
    };
    Vector2D.FromPoints = function (fromPoint, toPoint) {
        var ret = new Vector2D();
...
var Neuron = /** @class */ (function () {
    ...
    Neuron.Draw = function (theGame, neuron, ctx) {
        var draw = true;
        ...
        if (draw) {
            ...
            ctx.quadraticCurveTo(neuron.location.x + scaledPoints[1].x, neuron.location.y + scaledPoints[1].y, neuron.location.x + scaledPoints[2].x, neuron.location.y + scaledPoints[2].y);
            ctx.quadraticCurveTo(neuron.location.x + scaledPoints[3].x, neuron.location.y + scaledPoints[3].y, neuron.location.x + scaledPoints[4].x, neuron.location.y + scaledPoints[4].y);
            ctx.quadraticCurveTo(neuron.location.x + scaledPoints[5].x, neuron.location.y + scaledPoints[5].y, neuron.location.x + scaledPoints[6].x, neuron.location.y + scaledPoints[6].y);
            ctx.quadraticCurveTo(neuron.location.x + scaledPoints[7].x, neuron.location.y + scaledPoints[7].y, neuron.location.x + scaledPoints[8].x, neuron.location.y + scaledPoints[8].y);
            ctx.quadraticCurveTo(neuron.location.x + Neuron.neuronDrawPoints[9].x, neuron.location.y + Neuron.neuronDrawPoints[9].y, neuron.location.x + scaledPoints[0].x, neuron.location.y + scaledPoints[0].y);
            ctx.stroke();
        }
    };
    Neuron.Revive = function (neuron) {
        // Revive the neuron
        neuron.health = 100;
        neuron.ReviveTicks = (new Date()).getTime();
        neuron.WobbleAngle = 0;
    };
    Neuron.neuronDrawPoints = [{ x: 0, y: 7 }, { x: 1, y: 1 }, { x: 8, y: 8 }, { x: 1.5, y: 0.5 }, { x: 8, y: -8 }, { x: -1, y: -1 }, { x: 0, y: -8 }, { x: -1, y: -1 }, { x: -12, y: 2 }, { x: -1, y: 1 }];
    return Neuron;
}());
var Game = /** @class */ (function () {
    function Game(cv, gameOverCb) {
        this.deltaT = 1000;
        this.neurons = [];
        this.gameTicks = 0;
        this._age = 0;
    ...
    Game.mainLoop = function (deltaTms) {
    ...
    Game.prototype.initaliseSimplePaths = function () {
    ...
    Game.prototype.initaliseBrainPaths = function () {
        ...
        for (var count = 0; count < len; count++) {
            ...
            var connection = { point1: existing.point1 + 68, point2: existing.point2 + 68 };
            this.connections.push(connection);
        }
    };
    Game.prototype.initaliseNeurons = function (num) {
    ...
    Game.prototype.initalisePlayer = function () {
    ...
    Game.prototype.DrawGameData = function (ctx) {
    ...
    Game.prototype.DrawCircle = function (ctx, point, diameter) {
        var radius = diameter / 2;
        ctx.beginPath();
        ctx.ellipse(point.x, point.y, radius, radius, 0, 0, Math.PI * 2);
        ctx.stroke();
    };
    Game.prototype.DrawCircleLine = function (ctx, point1, point2, diameter) {
        var radius = diameter / 2;
    ...
    Game.FindBestConnectionId = function (theGame, fromPointId, pullVector) {
    ...
    Game.moveInDirection = function (theGame, pullToVect, pullToPoint, distTomove, distMultiplier) {
    ...
    Game.UpdateNeurons = function (theGame, detltTMs) {
        if (detltTMs > 0) {
    ...
    Game.DrawBoard = function (theGame) {
        var ctx = theGame.canvas.getContext('2d');
    ...
    ;
    Game.DetectEnd = function (theGame) {
    ...
    Game.PlayerPixelsPerSecSpeed = 100.0;
    Game.InitialNeuronHealthDeductionPerSecond = 2.0;
    Game.deathAccelerationFactor = 0.00001;
    return Game;
}());
//# sourceMappingURL=Neuronic.js.map